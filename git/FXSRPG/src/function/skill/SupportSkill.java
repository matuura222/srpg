 ...
 * @author 樹麗
 */
public class SupportSkill extends Skill implements IgetStateListable {
	 ...
	 */
	public SupportSkill(@JsonProperty("id")String id, @JsonProperty("name")String name,
			...
			@JsonProperty("state_list")List<UnitState> state_list, @JsonProperty("exp")int exp,
	...
	public void action(BattleState battle, Unit unit, List<Square> target_list) {
		...
		for(Square square : target){
			...
			square.getUnit().currentState(battle, ActivateType.ALWAYS);
		}
	...
	/**
	 * @return サポートスキルの基本値を返す
	 */
	public static SupportSkill defaultData() {
		return new SupportSkill("", "", "", SkillType.MATERIAL, 0, 0, 0, 0, 0, TargetType.ENEMY, new ArrayList<UnitState>(), 0, "");
	}
	/* (非 Javadoc)
	 * @see function.GameData#getDefaultData()
	 */
	@Override
	public SupportSkill getDefaultData() {
		return defaultData();
	}
}
